...
export default function NewPurchaseOrder() {
  ...
  const [header, setHeader] = useState({
    supplier_id: '',
    purchase_order_number: makePoNumber(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!header.purchase_order_number.trim()) throw new Error('PO number is required');
      if (!header.order_date) throw new Error('Order date is required');
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Add a new local purchase order</p>
        </div>
      </div>
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex space-x-2 pt-4 border-t border-gray-200">
              <button type="button" onClick={() => navigate('/purchases/orders')} className="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50">
                Cancel
              </button>
