...
export default function NewPurchaseInvoice() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!header.bill_number.trim()) throw new Error('Bill number is required');
      if (!header.bill_date) throw new Error('Bill date is required');
      ...
      const { error: itemInsertError } = await supabase.from('purchase_invoice_items').insert(
        cleanItems.map((li) => ({
      ...
      if (itemInsertError) {
        await supabase.from('purchase_invoices').delete().eq('id', bill.id);
        throw itemInsertError;
      }

      navigate(`/purchases/invoices/${bill.id}`);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to create purchase invoice');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Add a new supplier bill</p>
        </div>
      </div>
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex space-x-2 pt-4 border-t border-gray-200">
              <button type="button" onClick={() => navigate('/purchases/invoices')} className="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50">
                Cancel
              </button>
              <button type="submit" disabled={loading} className="px-6 py-2 bg-black text-white rounded-md hover:bg-gray-800 disabled:opacity-50">
                {loading ? 'Saving...' : 'Create Purchase Invoice'}
              </button>
            </div>
          </form>
        </div>
