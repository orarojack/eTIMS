...
type PurchaseInvoiceStatus = 'unpaid' | 'paid' | 'cancelled';
...
function mapToLineItem(li: PurchaseInvoiceItem): LineItem {
  return {
    item_id: li.item_id || '',
...
export default function PurchaseInvoiceDetail() {
  ...
  const [doc, setDoc] = useState<PurchaseInvoiceHeader | null>(null);
  const [docItems, setDocItems] = useState<PurchaseInvoiceItem[]>([]);
  ...
  useEffect(() => {
    void (async () => {
      ...
      setError('');
      try {
        if (!id) throw new Error('Missing purchase invoice id');
        ...
        const { data: docData, error: docError } = await supabase
          ...
          .select(
            'id,supplier_id,bill_number,bill_date,due_date,subtotal,discount_percentage,discount_amount,tax_amount,total,terms_and_conditions,status,created_at'
          )
          .eq('id', id)
          .single();
        ...
        setDoc(docData as PurchaseInvoiceHeader);
        setDocItems((itemsData || []) as PurchaseInvoiceItem[]);

        setHeader({
          supplier_id: docData.supplier_id,
          bill_number: docData.bill_number,
          bill_date: docData.bill_date,
          ...
          status: (docData.status || 'unpaid') as PurchaseInvoiceStatus,
        });
        setLineItems(((itemsData || []) as PurchaseInvoiceItem[]).map(mapToLineItem));
      } catch (e: unknown) {
  ...
  const handleSave = async () => {
    ...
    if (!header.bill_number.trim()) return setError('Bill number is required');
    ...
    try {
      const { data: updated, error: updateError } = await supabase
        ...
        .update({
          ...
          bill_number: header.bill_number.trim(),
      ...
      const { error: deleteItemsError } = await supabase.from('purchase_invoice_items').delete().eq('purchase_invoice_id', id);
      if (deleteItemsError) throw deleteItemsError;

      const { error: insertItemsError } = await supabase.from('purchase_invoice_items').insert(
        cleanItems.map((li) => ({
      ...
      setDoc(updated as PurchaseInvoiceHeader);
      setDocItems((itemsData || []) as PurchaseInvoiceItem[]);
      setMode('view');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to update purchase invoice');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!id) return;
    const ok = window.confirm('Delete this purchase invoice? This cannot be undone.');
    if (!ok) return;
    setDeleting(true);
    setError('');
    try {
      const { error: deleteError } = await supabase.from('purchase_invoices').delete().eq('id', id);
      if (deleteError) throw deleteError;
      navigate('/purchases/invoices');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to delete purchase invoice');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/purchases/invoices')} className="text-gray-600 hover:text-gray-900">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {mode === 'edit' ? 'Edit Purchase Invoice' : doc?.bill_number || 'Purchase Invoice'}
            </h1>
            <p className="text-gray-600 mt-1">{mode === 'edit' ? 'Update supplier bill' : `Supplier: ${supplierName}`}</p>
          </div>
        </div>
                  ...
                  onClick={() => {
                    if (doc) {
                      setHeader({
                        ...
                        status: (doc.status || 'unpaid') as PurchaseInvoiceStatus,
                      });
      ...
      <div className="bg-white rounded-lg shadow">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">Purchase invoice not found.</div>
        ) : mode === 'edit' ? (
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FieldShell label="Bill Number" required>
                <TextInput value={header.bill_number} onChange={(e) => setHeader({ ...header, bill_number: e.target.value })} />
              </FieldShell>
              <FieldShell label="Bill Date" required>
                <TextInput type="date" value={header.bill_date} onChange={(e) => setHeader({ ...header, bill_date: e.target.value })} />
              </FieldShell>
              <FieldShell label="Due Date">
                <TextInput type="date" value={header.due_date} onChange={(e) => setHeader({ ...header, due_date: e.target.value })} />
              </FieldShell>
              ...
              <div className="md:col-span-2">
                <FieldShell label="Status">
                  <SelectInput value={header.status} onChange={(e) => setHeader({ ...header, status: e.target.value as PurchaseInvoiceStatus })}>
          ...
          <div ref={printRef} className="bg-white p-8 max-w-4xl mx-auto rounded-lg shadow print:shadow-none print:p-0">
            <div className="print:block">
              ...
              <div className="flex justify-between items-start gap-8 mb-8">
                <div>
                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">Supplier</div>
                  <div className="font-semibold text-gray-900">{supplierName}</div>
                  <div className="text-sm text-gray-600 mt-1">Vendor</div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-gray-900">PURCHASE INVOICE</div>
                  <div className="text-sm text-gray-600 mt-2">
                    <div>Bill No: {doc.bill_number}</div>
                    <div>Date: {new Date(doc.bill_date).toLocaleDateString()}</div>
                    {doc.due_date && <div>Due: {new Date(doc.due_date).toLocaleDateString()}</div>}
                ...
                <div className="w-64 space-y-1 text-sm">
                  {doc.discount_amount > 0 && (
                    <div className="flex justify-between">
                      ...
                      <span>{formatMoney(-doc.discount_amount, 'KES')}</span>
                    </div>
                  )}
                  {doc.tax_amount > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Tax</span>
                      <span>{formatMoney(doc.tax_amount, 'KES')}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-lg font-bold pt-2 border-t border-gray-300">
                    <span>Total</span>
                    <span>{formatMoney(doc.total, 'KES')}</span>
                  </div>
                </div>
              </div>
              {doc.terms_and_conditions && (
                <div className="mt-8 pt-6 border-t border-gray-200">
                  ...
                  <div className="text-sm text-gray-700 whitespace-pre-wrap">{doc.terms_and_conditions}</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div className="font-medium text-gray-900 mt-1">{new Date(doc.bill_date).toLocaleDateString()}</div>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
