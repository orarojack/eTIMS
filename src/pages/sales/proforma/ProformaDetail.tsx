...
import { getPublicProformaViewUrl } from '../../../services/publicInvoiceUrl';
...
type ProformaStatus = 'draft' | 'issued' | 'paid' | 'cancelled';
...
function mapToLineItem(li: ProformaItem): LineItem {
  return {
    item_id: li.item_id || '',
...
export default function ProformaDetail() {
  ...
  const [doc, setDoc] = useState<ProformaHeader | null>(null);
  const [docItems, setDocItems] = useState<ProformaItem[]>([]);
  const [docCustomer, setDocCustomer] = useState<InvoiceDocumentCustomer | null>(null);
  ...
  const [header, setHeader] = useState({
    ...
    status: 'draft' as ProformaStatus,
  });
    ...
    void (async () => {
      ...
      try {
        ...
        const { data: docData, error: docError } = await supabase
          ...
          .select(
            'id,customer_id,proforma_number,proforma_date,sale_type,tax_type,subtotal,discount_percentage,discount_amount,tax_amount,total,terms_and_conditions,payment_method,status,created_at,qr_code'
          )
          .eq('id', id)
          .single();
        if (docError) throw docError;

        let effectiveQr = docData.qr_code as string | null;
        if (!effectiveQr?.trim()) {
          const next = crypto.randomUUID();
          const { error: patchQrError } = await supabase.from('proforma_invoices').update({ qr_code: next }).eq('id', id);
        ...
        setDoc({ ...(docData as ProformaHeader), qr_code: effectiveQr });
        setDocItems((itemsData || []) as ProformaItem[]);

        setHeader({
          customer_id: docData.customer_id,
          proforma_number: docData.proforma_number,
          ...
          status: (docData.status || 'draft') as ProformaStatus,
        });
        setLineItems(((itemsData || []) as ProformaItem[]).map(mapToLineItem));
      } catch (e: unknown) {
  ...
  const handleSave = async () => {
    ...
    if (!header.proforma_number.trim()) return setError('Proforma number is required');
    ...
    try {
      const { data: updated, error: updateError } = await supabase
        ...
        .update({
          ...
          proforma_number: header.proforma_number.trim(),
          proforma_date: header.proforma_date,
      ...
      const { error: deleteItemsError } = await supabase.from('proforma_items').delete().eq('proforma_id', id);
      if (deleteItemsError) throw deleteItemsError;

      const { error: insertItemsError } = await supabase.from('proforma_items').insert(
        cleanItems.map((li) => ({
          proforma_id: id,
          ...li,
        }))
      ...
      setDoc(updated as ProformaHeader);
      setDocItems((itemsData || []) as ProformaItem[]);
      setMode('view');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to update proforma');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!id) return;
    const ok = window.confirm('Delete this proforma? This cannot be undone.');
    if (!ok) return;
    setDeleting(true);
    setError('');
    try {
      const { error: deleteError } = await supabase.from('proforma_invoices').delete().eq('id', id);
      if (deleteError) throw deleteError;
      navigate('/sales/proforma');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to delete proforma');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="no-print flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/sales/proforma')} className="text-gray-600 hover:text-gray-900">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {mode === 'edit' ? 'Edit Proforma' : doc?.proforma_number || 'Proforma'}
            </h1>
            <p className="text-gray-600 mt-1">{mode === 'edit' ? 'Update proforma' : `Customer: ${customerName}`}</p>
          </div>
        </div>
                  ...
                  onClick={() => {
                    if (doc) {
                      setHeader({
                        ...
                        proforma_number: doc.proforma_number,
                        ...
                        status: (doc.status || 'draft') as ProformaStatus,
                      });
      ...
      >
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">Proforma not found.</div>
        ) : mode === 'edit' ? (
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FieldShell label="Proforma Number" required>
                <TextInput value={header.proforma_number} onChange={(e) => setHeader({ ...header, proforma_number: e.target.value })} />
              </FieldShell>
              <FieldShell label="Proforma Date" required>
                <TextInput type="date" value={header.proforma_date} onChange={(e) => setHeader({ ...header, proforma_date: e.target.value })} />
              </FieldShell>
              ...
              <div className="md:col-span-2">
                <FieldShell label="Status">
                  <SelectInput value={header.status} onChange={(e) => setHeader({ ...header, status: e.target.value as ProformaStatus })}>
          ...
          <InvoiceDocumentShell ref={printRef}>
            <InvoiceDocumentTemplate
              ...
              invoice={{
                invoice_number: doc.proforma_number,
                invoice_date: doc.proforma_date,
                due_date: null,
              ...
              invoiceItems={docItems}
              qrUrl={doc.qr_code?.trim() ? getPublicProformaViewUrl(doc.qr_code) : null}
            />
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div className="font-medium text-gray-900 mt-1">{new Date(doc.proforma_date).toLocaleDateString()}</div>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
