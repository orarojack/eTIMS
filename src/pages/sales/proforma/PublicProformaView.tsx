...
function getErrorMessage(e: unknown): string {
  ...
  return 'Failed to load proforma';
}
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        const { data: raw, error: rpcError } = await supabase.rpc('get_public_proforma', {
          p_token: resolvedToken,
        });
        if (rpcError) {
          setError(
            [rpcError.message, rpcError.details].filter(Boolean).join(' — ') || 'Could not load proforma from server'
          );
          return;
        }
  ...
  if (error || !payload) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <p className="text-gray-700">{error || 'Proforma not found'}</p>
      </div>
    );
  }

  const qrUrl = resolvedToken ? getPublicProformaViewUrl(resolvedToken) : null;
