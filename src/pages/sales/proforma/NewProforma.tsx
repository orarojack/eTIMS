...
export default function NewProforma() {
  ...
  const [header, setHeader] = useState({
    customer_id: '',
    proforma_number: makeProformaNumber(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!header.proforma_number.trim()) throw new Error('Proforma number is required');
      if (!header.proforma_date) throw new Error('Proforma date is required');
      ...
      const { error: itemInsertError } = await supabase.from('proforma_items').insert(
        cleanItems.map((li) => ({
      ...
      navigate(`/sales/proforma/${proforma.id}`);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to create proforma');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Create Proforma</h1>
          <p className="text-gray-600 mt-1">Add a new proforma invoice</p>
        </div>
      </div>
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FieldShell label="Proforma Date" required>
                <TextInput
                  required
                  type="date"
                  value={header.proforma_date}
                  onChange={(e) => setHeader({ ...header, proforma_date: e.target.value })}
                />
              </FieldShell>
                  ...
                  <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                    ...
                    <div className="md:col-span-3">
                      <FieldShell label="Product name" required>
                        <TextInput required value={li.product_name} onChange={(e) => updateLine(index, { product_name: e.target.value })} />
                      </FieldShell>
                    </div>
                    <div className="md:col-span-2">
                      <FieldShell label="Qty" required>
                        <TextInput required type="number" min="0" step="0.01" value={li.quantity} onChange={(e) => updateLine(index, { quantity: e.target.value })} />
                      </FieldShell>
                    </div>
                    <div className="md:col-span-2">
                      <FieldShell label="Unit price" required>
                        <TextInput required type="number" min="0" step="0.01" value={li.unit_price} onChange={(e) => updateLine(index, { unit_price: e.target.value })} />
                      </FieldShell>
                    </div>
                    <div className="md:col-span-2 flex items-center justify-between gap-3">
                      <div className="text-sm text-gray-600">
                        ...
                        <span className="font-medium text-gray-900">
                          {Number(totals.parsed[index]?.amount || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                        </span>
                      </div>
            ...
            <div className="flex space-x-2 pt-4 border-t border-gray-200">
              <button type="button" onClick={() => navigate('/sales/proforma')} className="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50">
                Cancel
              </button>
              <button type="submit" disabled={loading} className="px-6 py-2 bg-black text-white rounded-md hover:bg-gray-800 disabled:opacity-50">
